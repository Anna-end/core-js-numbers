...
function roundToPowerOfTen(num, pow) {
 return pow === 0 ? num : 
        pow === 1 ? (Math.round( num/10)) * 10 :
        pow === 2 ? (Math.round( num/100)) * 100 :
...
function getSumOfDigits( num ) {
  let arrNums = Array.from(String(num), Number);
  let sum = 0;
  arrNums.forEach(function(arrNum) {
    sum += arrNum;
  })
  return sum;
}
...
function isPowerOfTwo( num ) {
 return Math.sqrt(num) % 2 == 0 ? true : false;
}
...
function getSumOfNumbers(x1, x2, x3 ) {
  let result = x1 + x2 + x3;
  return result <= 0 ? result.toFixed(1) : result;
}
